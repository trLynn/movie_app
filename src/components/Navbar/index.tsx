...
function Navbar() {
    ...
    const [searchedList, setSearchList] = useState<CarouselMovie[]>([])
    ...
    const fetchSearch = async () => {
        try {
            const response = await baseApi.get(`/3/search/movie?query=${search}&include_adult=true&language=en-US&page=1`)
    ...
    return (
        <nav className="bg-[#121212] py-2">
            <div className="flex justify-content-between items-center lg:w-[80%] md:w-[90%] w-[95%] mx-auto">
                ...
                <div className="relative">
                    <input
                        placeholder="search"
                        className="md:w-[500px] sm:w-[350px] w-[180px] h-10 bg-black text-[#c2c2c2] md:text-lg sm:text-md text-sm outline-none sm:px-4 px-3 placeholder:text[#646464] rounded-xl"
